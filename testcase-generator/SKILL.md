...
|------|------|
| 이미지 | `output/images/` |
| PPTX 데이터 | `output/pptx_data.json` |
| 청크 계획 | `output/chunk_plan.json` |
| 청크별 TC | `output/tc_chunk_*.json` |
| 병합 TC | `output/tc_data.json` |
| Excel 결과 | `output/{프로젝트명}_TC.xlsx` |
...
**영역명 예시**:
- 좌측: Common Tool, Object List, Navigator
- 우측: Task Tool, Property Panel
- 상단: 단계 버튼 (Alignment, Orientation 등), 타이틀바
- 하단: 상태바, 진행률 표시 영역

#### 3. Expected Result 단축키 통합
기능 설명 후 단축키 정보를 **함께 포함**:
```
...
#### 4. 이미지 분석 강화
이미지에서 파악하여 TC에 반영할 정보:
- **UI 영역 구분**: 좌측/우측/상단/하단 영역 식별
- **버튼 배치**: Common Tool 버튼 순서 및 위치
- **단축키 힌트**: 버튼 Tooltip에 표시된 단축키 정보
- **아이콘 상태**: On/Off 상태 시각적 변화 (색상, 배경)
